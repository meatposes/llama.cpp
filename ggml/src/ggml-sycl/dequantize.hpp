...
static void dequantize_block_q2_0_reorder(const void * __restrict__ vx, dst_t * __restrict__ yy, int64_t k,
                                  const sycl::nd_item<3> &item_ct1) {
    // One qs byte (4 elements) per work-item. Adjacent lanes read adjacent bytes and write
    // adjacent 4-element runs, so both sides coalesce. Assigning a whole QK2_0 block per lane
    // instead makes each lane write QK2_0 elements apart, putting every store in a sub-group on
    // its own cache line.
    const int64_t nbytes = k / 4;  // Q2_0 SoA: qs region is [nblocks * QK2_0/4] = k/4 bytes
    const int64_t bi     = item_ct1.get_group(2) * item_ct1.get_local_range(2) + item_ct1.get_local_id(2);

    if (bi >= nbytes) {
        return;
    }

    const uint8_t      b     = ((const uint8_t *) vx)[bi];
