...
constexpr int MSTRIP=8;      // m-tiles per sub-group (M covered = 8*8 = 64)
...
int main(int argc,char**argv){
    ...
    auto run=[&](){ return q.submit([&](handler&h){
        ...
        local_accessor<int32_t,1> ST(WG_SG*TM*TN,h);
        ...
        h.parallel_for(nd_range<2>(global,local),[=](nd_item<2> it)[[sycl::reqd_sub_group_size(SG)]]{
            ...
            int8_t *B=&Bs[sgid*SUB*TN]; int32_t *st=&ST[sgid*TM*TN]; float *fa=&FA[sgid*MSTRIP*TM*TN];
            float *dwl=&dwL[sgid*TN]; float *dxl=&dxL[sgid*TM];
            for(int e=sl;e<MSTRIP*TM*TN;e+=SG) fa[e]=0.f;
            ...
            for(int s=0;s<NSUB;++s){
                ...
                for(int mi=0;mi<MSTRIP;++mi){
                    ...
                    for(int e=sl;e<TM*TN;e+=SG){ int m=e/TN,n=e%TN; fa[mi*TM*TN+e]+=dxl[m]*dwl[n]*(float)st[e]; }
                }
                sycl::group_barrier(sg);
            }
            for(int mi=0;mi<MSTRIP;++mi){ int mt=(m0/TM)+mi;
                for(int e=sl;e<TM*TN;e+=SG){ int m=e/TN,n=e%TN; dC[(size_t)(nt*TN+n)*Mtok+(mt*TM+m)]=fa[mi*TM*TN+e]; } }
        });
    }); };
    run().wait();
